...
for col in salaries_header:
	...
	col[7] = str("7x of Salary")
	col[9] = str("Chance of 7x")
...
for row in players_playing:
	salary_x7 = (float(row[4])/1000) * 7
	row[7] = str(salary_x7)
	players_with_7x.append(row)
...
with open('out_bb_tourney.csv', 'w', newline="") as f:
	writer = csv.writer(f)
	writer.writerows(salaries_header)
	writer.writerows(players_with_high_ceilings)